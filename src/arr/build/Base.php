...
namespace cylcode\tools\arr\build;
...
{
    /**
     * [forMat 格式化数组 ，指定KEY和VALUE ，如果不指定，直接选前两个KEY]
     * @Author   Jerry                    (wx621201)
     * @DateTime 2019-05-31T17:43:17+0800
     * @Example  eg:
     ...
     */
    public function forMat($data,$key='',$value=''){
        $kstr = $key;
        $vstr = $value;
        unset($key,$value);
        if(!$data) return [];
        $newData = [];
        foreach ($data as  $v) {
            if(!$kstr)$kstr = array_keys($v)[0];
            if(!$vstr)$vstr = array_keys($v)[1];
            $newData[$v[$kstr]]=$v[$vstr];
        }
        return $newData;
    }
    ...
    /**
     * [selectSearchForMat select group 数据格式化]
     * @Author   Jerry                    (wx621201)
     * @DateTime 2019-06-04T09:41:35+0800
     * @Example  eg:
     * @param    [type]                   $data      [description]
     ...
     * @return   [type]                              [description]
     */
    public function selectSearchForMat($data,$name='',$value=''){
        if(!$data) return [];
        $newData = [];
        foreach ($data as  $v) {
            if(!$name)$name = array_keys($v)[0];
            if(!$value)$value = array_keys($v)[1];
            $newData[]=[
                'name' => $v[$name],
                'value'=>$v[$value]
            ];
     ...
     * [objToArray 对象转数组]
     * @Author   Jerry                    (wx621201)
     * @DateTime 2019-05-31T17:41:54+0800
     * @Example  eg:
